...
import { faTrashAlt, faEdit, faBuilding, faInfoCircle, faLink, faExternalLinkAlt } from '@fortawesome/free-solid-svg-icons';
...
const ListaEmpresasExternas = () => {
  ...
  const addEmpresasToList = (newEmpresa) => {
    setEmpresasExternas(prevEmpresas => [...prevEmpresas, newEmpresa]);

  };
  const [showSuccessEditModal, setShowSuccessEditModal] = useState(false);

  const onEmpresaUpdated = (empresaActualizada) => {
    const empresasActualizadas = empresasExternas.map((empresa) => {
        if (empresa._id === empresaActualizada._id) {
  ...
  return (
    <div className="App">
      <TabsAdministracionComp onAddEmpresa={addEmpresasToList} />
      ...
      {showEditModal && empresaExternaToEdit && (
        <Modal show={showEditModal} onHide={() => setShowEditModal(false)} size="lg">
          ...
          <Modal.Body>
            ...
            <EditarEmpresaExterna id={empresaExternaToEdit} onEmpresaUpdated={onEmpresaUpdated} closeEditModal={() => setShowEditModal(false)} />
      ...
      <Modal show={showSuccessEditModal} onHide={() => setShowSuccessEditModal(false)}>
    <Modal.Header closeButton>
    ...
    <Modal.Footer>
        <Button variant="success" onClick={() => setShowSuccessEditModal(false)}>Cerrar</Button>
    </Modal.Footer>
</Modal>
    </div>
  );
